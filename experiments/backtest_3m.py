    ...
    for i, bar in enumerate(bars):
        ...
        if signal == "BUY" and position == 0 and capital > 0:
            ...
            if shares > 0:
                ...
                signals.append({'date': bar.timestamp, 'type': 'BUY', 'price': bar.close})
            ...
            capital += revenue
            trades.append(pnl)
            signals.append({'date': bar.timestamp, 'type': 'SELL', 'price': bar.close, 'pnl': pnl})
            position = 0
...
def main():
    ...
    print("="*80)
    print("🔬 科华生物 (002022.SZ) - 新策略最近 3 个月回测")
    print("="*80)
    ...
    # 新策略参数（从 config.yaml 读取）
    new_params = config['strategies']['profiles']['grid_combo_002022']
    print(f"\n⚙️  新策略参数:")
    ...
    if result['signals']:
        ...
        for sig in result['signals']:
            if sig['type'] == 'BUY':
                print(f"   {sig['date'].strftime('%Y-%m-%d')} 买入 @ {sig['price']:.2f} 元")
            else:
                print(f"   {sig['date'].strftime('%Y-%m-%d')} 卖出 @ {sig['price']:.2f} 元 (盈亏：{sig['pnl']:.2f} 元)")
    ...
    # 对比 1 年回测
    print(f"\n{'='*80}")
    print("📋 对比：3 个月 vs 1 年回测")
    print(f"{'='*80}")
    print(f"   3 个月收益：{result['total_return']:.2f}%")
    print(f"   1 年收益（之前最优）：35.81%")
    print(f"   年化对比：{(1 + result['total_return']/100)**(12/3) - 1:.2%} vs 35.81%")
