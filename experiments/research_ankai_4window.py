...
def create_strategies() -> List[Tuple[str, Any]]:
    ...
    # 1. MA 交叉策略（多参数）
    for fast, slow in [(5, 20), (8, 21), (10, 30), (5, 15), (10, 25), (3, 13), (13, 48), (5, 13), (8, 15)]:
    ...
    # 6. ATR 通道策略（多参数）
    for period, mult in [(14, 2.0), (14, 2.5), (20, 2.0), (10, 2.0), (14, 1.8), (14, 2.2), (18, 2.0), (14, 3.0)]:
        strategies.append((f"ATR_{period}_{mult}", ATRChannel(period=period, multiplier=mult)))
    
    # 7. MACD 策略（多参数变体）
    for fast, slow, signal in [(12, 26, 9), (8, 21, 5), (10, 24, 8), (6, 19, 5),
                               (9, 22, 7), (11, 26, 8), (10, 20, 7), (8, 17, 5)]:
    ...
    for params in [
        ...
        (5, 20, 6, 25, 65, 20, 1.8, 1),  # 当前安凯配置
        (5, 15, 6, 20, 60, 20, 2.0, 1),  # 科华优化配置
        (5, 13, 6, 25, 65, 20, 1.8, 1),
        (8, 20, 6, 25, 65, 20, 1.8, 1),
        (5, 18, 6, 25, 65, 20, 1.8, 1),
    ]:
        gc = GridCombo(*params)
    ...
    # 15. Donchian + MA 趋势过滤
    class DonchianMAFilter:
        def __init__(self, donchian_window, ma_fast, ma_slow):
            self.donchian = DonchianBreakout(window=donchian_window)
            self.ma = MovingAverageCross(fast=ma_fast, slow=ma_slow)
            self.name = f"DonchianMA_{donchian_window}_{ma_fast}_{ma_slow}"
        
        def on_bar(self, bar):
            d_sig = self.donchian.on_bar(bar)
            ma_sig = self.ma.on_bar(bar)
            
            # Donchian 突破 + MA 多头趋势 → BUY
            if d_sig.side == Side.BUY and ma_sig.side == Side.BUY:
                return Signal(timestamp=bar.timestamp, side=Side.BUY, strength=0.9)
            # Donchian 突破 + MA 空头趋势 → SELL
            if d_sig.side == Side.SELL and ma_sig.side == Side.SELL:
                return Signal(timestamp=bar.timestamp, side=Side.SELL, strength=0.9)
            # 否则跟随 Donchian
            return d_sig
    ...
    for params in [(20, 5, 20), (15, 5, 15), (20, 8, 21), (18, 5, 18)]:
        dmf = DonchianMAFilter(*params)
        strategies.append((dmf.name, dmf))
    
    return strategies


def run_4window_research(
    symbol: str = "000868.SZ",
    name: str = "安凯客车",
) -> Dict[str, Any]:
    """
    运行四窗口策略研究
...
def save_report(report: Dict[str, Any], output_path: Optional[Path] = None):
    ...
    if output_path is None:
        output_path = ROOT / 'data' / 'backtest' / 'ankai_4window_research.json'
...
def generate_markdown_summary(report: Dict[str, Any], current_strategy_info: Optional[Dict] = None):
    """生成 Markdown 格式总结"""
    output_path = ROOT / 'reports' / 'ankai_4window_research_summary.md'
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
"""
    ...
    print(f"\n📄 Markdown 总结已保存：{output_path}")


def backtest_current_strategy(symbol: str = "000868.SZ") -> Dict[str, Any]:
    """
    回测当前配置的策略
    ...
    from ripple_tradePilot.strategies.rsi import RSI
    from ripple_tradePilot.strategies.bollinger import BollingerBands
    from ripple_tradePilot.models.types import Side, Signal
    
    class CurrentGridCombo:
        def __init__(self):
            self.ma = MovingAverageCross(fast=5, slow=20)
            self.rsi = RSI(period=6, oversold=25, overbought=65)
            self.bb = BollingerBands(period=20, std_dev=1.8)
            self.name = "Grid_5_20_6_25_65_20_1.8_1 (current)"
    ...
    current_strategy = CurrentGridCombo()
    ...
    for key, days in HORIZONS:
        ...
        metrics = backtest_strategy(current_strategy, bars)
        results[key] = asdict(metrics)
...
def main():
    """主函数"""
    # 运行四窗口研究
    report = run_4window_research(symbol="000868.SZ", name="安凯客车")
    ...
    # 回测当前策略用于对比
    print("\n📊 回测当前配置策略用于对比...")
    current_results = backtest_current_strategy("000868.SZ")
    
    current_info = {}
    if '1y' in current_results:
        current_info['1y_return'] = current_results['1y']['total_return']
        current_info['1y_drawdown'] = current_results['1y']['max_drawdown']
        current_info['1y_trades'] = current_results['1y']['total_trades']
    
    # 生成 Markdown 总结（含对比）
    generate_markdown_summary(report, current_info if current_info else None)
    
    # 保存当前策略回测结果
    current_output = ROOT / 'data' / 'backtest' / 'ankai_current_strategy.json'
    current_output.parent.mkdir(parents=True, exist_ok=True)
    with open(current_output, 'w', encoding='utf-8') as f:
        json.dump({'current_strategy': 'Grid_5_20_6_25_65_20_1.8_1', 'periods': current_results}, f, ensure_ascii=False, indent=2)
    
    print(f"\n✅ 当前策略回测已保存：{current_output}")
